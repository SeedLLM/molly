...
# 加载 parquet 文件
val_df = pd.read_parquet('/tos-bjml-ai4agr/lijinzhe/dataset/BioMLLM/TargetTask0808/val_10_task.parquet')
train_df = pd.read_parquet('/tos-bjml-ai4agr/lijinzhe/dataset/BioMLLM/TargetTask0808/train_10_task.parquet')
...
if not duplicates_in_train.empty:
    ...
    # 打印 val 和 train 中重复的 input 和 task
    for index, row in duplicates_in_train.iterrows():
        val_input = row['input']
        val_task = row['task']
        # 获取 train_df 中匹配的行
        matching_train_row = train_df[train_df['input'] == val_input].iloc[0]
        train_task = matching_train_row['task']
        train_input = matching_train_row['input']
        
        print(f"val_df: input = {val_input}, task = {val_task}")
        print(f"train_df: input = {train_input}, task = {train_task}")
        print("-" * 50)  # 分隔符，方便查看
else:

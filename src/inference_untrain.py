...
from dataset.dna_rna_dataset import (
    DatasetConfig,
    OmicsTestDataset,
    qwen_dna_collate_fn_inference,
)
...
def parse_args():
    ...
    parser.add_argument(
        "--text_model_path", type=str, required=True, help="Path to text model (Qwen)"
    )
    parser.add_argument(
        "--bio_model_path", type=str, required=True, help="Path to bio model"
    )
    parser.add_argument(
        "--multimodal_k_tokens",
        type=int,
    ...
    parser.add_argument(
        "--batch_size", type=int, default=8, help="Batch size for dataset inference"
    )
...
class MultiModalInfer:
    ...
    def __init__(self, args):
        ...
        self.device = torch.device(
            "cuda:1" if args.device == "cuda" and torch.cuda.is_available() else "cpu"
        )
        ...
        self.bio_tokenizer = AutoTokenizer.from_pretrained(
            args.bio_model_path, trust_remote_code=True
        )
    ...
    def _load_model(self):
        ...
        qwen_model = AutoModelForCausalLM.from_pretrained(
            args.text_model_path, trust_remote_code=True, torch_dtype=torch.bfloat16
        )
        ...
        bio_model = AutoModelForMaskedLM.from_pretrained(
            args.bio_model_path, trust_remote_code=True
        )

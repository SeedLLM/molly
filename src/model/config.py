    ...
    def __post_init__(self):
        ...
        if isinstance(self.text_config, dict):
            self.text_config = AutoConfig.from_pretrained(
                self.text_config["model_name"], **self.text_config.get("config", {})
            )
        if isinstance(self.dna_rna_config, dict):
            self.dna_rna_config = EsmConfig.from_pretrained(
                self.dna_rna_config["model_name"],
                **self.dna_rna_config.get("config", {})
            )
        if isinstance(self.protein_config, dict):
            self.protein_config = EsmConfig.from_pretrained(
                self.protein_config["model_name"],
                **self.protein_config.get("config", {})
            )
...
def get_omics_one_config(text_model_path, dna_rna_model_path, protein_model_path):
    ...
    dna_rna_config = AutoConfig.from_pretrained(
        dna_rna_model_path, trust_remote_code=True
    )
    protein_config = AutoConfig.from_pretrained(
        protein_model_path, trust_remote_code=True
    )
